...
A HTML document can be embeded in an inline frame if the HTTP response header X-Frame-Options is set to allow the embeding. There are three options the X-Frame-Options can be set to, resulting in different behavior. However, if X-Frame-Options is removed then the HTML document can be embeded anywhere.  
\begin{description}
 \item[DENY] \hfill \\
